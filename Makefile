...
REALLEMOCAMLLIBDIR=$(LEMDIR)ocaml-lib
LEMOCAMLLIBDIR=lem-ocaml-lib-for-cmm

ATOMICDIR=../axiomatic/ntc
OPERATIONALDIR=../axiomatic/ntc/operational
...
all: ocaml-lib bin/main web/cppmem.js
all-server: bin/main web/cppmem.js

# we make a local copy of the Lem ocaml-lib so that we can override the native pset implementation by one implemented using lists, to work around the problem with set equality
ocaml-lib: 
	rm -rf $(LEMOCAMLLIBDIR)/*
	cp $(REALLEMOCAMLLIBDIR)/*.ml* $(LEMOCAMLLIBDIR)
	cp $(REALLEMOCAMLLIBDIR)/Makefile $(LEMOCAMLLIBDIR)
	cp $(LEMOCAMLLIBDIR)/pset_using_lists.ml $(LEMOCAMLLIBDIR)/pset.ml
	cd $(LEMOCAMLLIBDIR) && make
...
# NB these dependencies are removed in the server copy, by fixmakefilefortar.sh
cmm.ml cmm.html opsem.ml opsem_conversion.ml: # $(ATOMICDIR)/cmm.lem $(OPERATIONALDIR)/opsem.lem $(OPERATIONALDIR)/opsem_conversion.lem 
	rm -f $@
	$(LEMDIR)/lem -html -ocaml -lib $(LEMDIR)/library $(ATOMICDIR)/cmm.lem $(OPERATIONALDIR)/opsem.lem $(OPERATIONALDIR)/opsem_conversion.lem
	chmod u-w $@
...
tar : clean cmm.ml ocaml-lib
	rm -rf cppmem # TODO: bad
	mkdir cppmem
	cp -R lem-ocaml-lib-for-cmm cppmem/lem-ocaml-lib-for-cmm
	cp _tags cppmem
